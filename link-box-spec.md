...
4. **If tagged `watch`** (movie/TV): run the movie enrichment sub-pipeline —
   ...
   - **An exact title match wins over the top result.** `/search/multi` is ordered by popularity, so taking the first hit hands a brand-new show to whatever older film or series shares its name and outranks it — a quiet, convincing failure that produces a real poster and a real rating for the wrong thing.
   - **If TMDb doesn't know the title, OMDb is asked directly** (`?t=`). OMDb used to be reachable only through an `imdb_id` that TMDb handed over, so a title TMDb hadn't indexed yet — or knew under another name — got nothing, even when OMDb had it all along. That path yields a rating, a synopsis, the `imdb_id` and a poster; no trailer, which OMDb doesn't carry.
   ...
   - **Some titles legitimately have no rating, and none is invented.** A show that has just come out has `vote_average: 0` on TMDb (nobody has voted, which is an absence rather than a score of zero) and `"N/A"` from OMDb, which withholds a rating below a vote threshold. Such a row gets its poster, synopsis and `imdb_id` and no number. The three no-rating outcomes are told apart by what *else* the row has: no poster and no tag of `watch` means it never entered this pipeline; no poster with the tag means neither service matched the title; a poster with no number means nobody has rated it yet. Each is logged at `info` so the deployment's logs say which.
...
- **Tapping a rating goes to IMDb, always.** With `imdb_id` it opens the title directly; without one it opens IMDb's search for the title's own name, which lands one tap away. Only a row with no title at all can't be pointed anywhere.
  The id was a *precondition* first, and that was wrong in a way worth recording: `imdb_id` is only filled by enrichment or the backfill route, so every row saved before those existed rendered as plain text, and tapping a rating did nothing. The badge had also been deliberately styled to look identical whether or not it linked — reasoning that an inconsistent affordance would make the unlinked ones look broken. The opposite happened: it made a working feature look broken, because a tap that silently does nothing is indistinguishable from a bug. An affordance that is sometimes absent is a smaller problem than one that is always present and sometimes inert.
