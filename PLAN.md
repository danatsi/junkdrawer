...
- **Search** (open q4) — proposal: a field revealed by a small icon in the header, matching
  title + domain + tags (including freeform) + note, client-side over the already-fetched list.
...
Shortcut POST /api/capture
   ...
   └─ waitUntil( enrich(row) ):
        ...
        2. Gemini Flash ──────────► { clean_title, summary, tags }
...
- Phase 6 proposals above (swipe, empty state, search, multi-expand) are proposals, not decisions —
  worth confirming when Phase 6 starts rather than now, since daily use will inform them.
